...
class easy_method(object):
    ...
    def set_io(self, prm, fulfill=True, append=True, sync=True):
        ...
        if sync:
            ...
            command = [{"command": "set_io_async",
                        "prm": prm, "fulfill": fulfill}]
    ...
    def play_gcode(self, gcode_path=None, gcode=None, **kwargs):
        ...
        # xyz space
        self.play({"command": "move", "prm": {"path": "line",
                  "movement": 1, "x": 0}}, append=False)
...
class Dorna(_port_usb, easy_method):
    ...
    def _bossac(self, port_name, bin_path):
        platform = sys.platform
        if platform == "win32":  # windows
            bossac_path = resource_filename(
                self._mn, 'resources/windows/bossac')
            bossac = [bossac_path, "-p", port_name, "-U",
                      "true", "-e", "-w", "-v", "-b", bin_path, "-R"]

        elif platform == "darwin":  # mac
            bossac_path = resource_filename(self._mn, 'resources/mac/bossac')
            bossac = ["sudo", bossac_path, "-U", "true",
                      "-e", "-w", "-v", "-b", bin_path, "-R"]
            ...
            bossac = ["sudo", bossac_path, "-p", port_name, "-U",
                      "true", "-e", "-w", "-v", "-i", "-b", bin_path, "-R"]
        # installing bossac
        line_last = ""

        self._log_add({"status": 1, "message": "Updating..."},
                      "update_firmware")
        time.sleep(1)
        try:
                ...
                for line in p.stdout:
                    ...
                    if all([x in line for x in ["(", "/", " pages)", "%"]]):
                        try:
                            ...
                            percentage_text = (
                                8 - len(str(_percentage) + "%")) * " " + str(_percentage) + "%"
                            # square 35 characters: " |███████████████████████████     |"
                            square_text = " |" + _number_sqr*"\u2588" + \
                                (_max_square - _number_sqr)*" " + "|"
                            # command 17 characters: " 108/111 commands"
                            _command_text = str(
                                nom) + "/"+str(denom) + " pages"
                            _command_text = (
                                17-len(_command_text))*" " + _command_text
                            if nom != denom:
                                print(percentage_text, square_text,
                                      _command_text,  end="\r")
                            else:
                                print(percentage_text,
                                      square_text, _command_text)
    ...
    def _bossac_reset(self, port_name):
        ...
        self._log_add({"status": 1, "message": "Reseting..."},
                      "update_firmware")
        time.sleep(1)
        try:
    ...
    def _bossac_exe(self, bossac_path):
        ...
        # run the command
        sp = Popen(bossac, shell=False, stdout=PIPE, stderr=PIPE,
                   bufsize=1, universal_newlines=True)
    ...
    def reset_board(self, port_name=None):
        ...
        """
        self._log_add({"status": 1, "message": "Progressing..."},
                      "update_firmware")
        print("Progressing...")
        ...
        if sys.platform not in ["win32", "darwin"]:  # linux
            ...
            sp = Popen(["sudo", "bossac"], shell=False, stdout=PIPE,
            ...
            for line in out + err:
                ...
                self._log_add({"status": 1, "message": line},
                              "update_firmware")
                if "not found" in line:
                    # log
                    _rtn = {
                        "status": 100, "message": "You need to install BOSSA flash programming utility. Run: sudo apt-get install bossa-cli"}
    ...
    def _reset_board(self, port_name):
        ...
        while i < num_try and not baud:
            ...
            if port_baud not in port_list_after:
                difference_list = [
                    x for x in port_list_after if x not in port_list_before]
                if difference_list:
            ...
            if baud:
                ...
                if port_bossac not in port_list_bossac:
                    difference_list = [
                        x for x in port_list_bossac if x not in port_list_after]
                    if difference_list:
                ...
                if self._bossac_reset(port_bossac):
                    # log
                    _rtn = {
                        "status": 0, "message": "Completed: board reseted on port " + port_name + "."}
                    self._log_add(_rtn, "update_firmware")
                    return json.dumps(_rtn)
    ...
    def update_firmware(self, port_name=None, firmware_path=None):
        ...
        if not firmware_path:
            firmware_path = resource_filename(
                self._mn, 'resources/firmware/firmware.bin')
        ...
        if port_name:
            result = json.loads(
                self._update_firmware(port_name, firmware_path))
            if not result["status"]:
                # log
                return result
        else:
        ...
        while i < num_try and not baud:
            ...
            if baud:
                ...
                if self._bossac(port_bossac, firmware_path):
                    # log
                    _rtn = {
                        "status": 0, "message": "Completed: firmware updated on port " + port_name+"."}
                    self._log_add(_rtn, "update_firmware")
                    return json.dumps(_rtn)
    ...
    def _print_percentage(self, nom, denom, new_line=False):
        ...
        _percentage = math.floor(100*nom/denom)
        # percentage 8 characters: "    100%"
        percentage_text = (8 - len(str(_percentage) + "%")
                           ) * " " + str(_percentage) + "%"
        # square 35 characters: " |███████████████████████████     |"
        square_text = " |" + _number_sqr*"\u2588" + \
            (_max_square - _number_sqr)*" " + "|"
        ...
        while len(command_list) and time.time() < max_time and self._device["connection"]:
            try:
                if command_list[0]["id"] <= self._system["command"][5][-1]["id"]:
                    ...
                    # add to the log
                    self._log_add(
                        {"nom": _init_nom, "denom": denom}, "connect_percentage")
                else:
                    time.sleep(0.02)
    ...
    def _connect_percentage_backup(self, _init_nom, denom, command_list, max_time):
        while len(command_list) and time.time() < max_time and self._device["connection"]:
            if command_list[0]["id"] <= self._system["command"][5][-1]["id"]:
                ...
                command_list.pop(0)
                # add to the log
                self._log_add({"nom": _init_nom, "denom": denom},
                              "connect_percentage")
            else:
                time.sleep(0.02)
    ...
    def _command_by_id(self, id_list):
        ...
        while id_list and command_index >= 0:
            result = next(
                (item for item in self._system["command"][command_index] if item["id"] == id_list[0]), None)
            if result != None:
    ...
    def _log_add(self, msg,  _type="info"):
        ...
        self._log.append(json.dumps(
            {"id": self._log_id, "time": time.time(), "type": _type,  "message": msg}))
        self._log_id += 1
    ...
    def _init_variable(self):
		...
		"""
        self._device = {"id": None, "connection": 0,  "port": None,
                        "fv": None, "config": None, "state": None, "version": "1.4.2"}
        ...
        # home j0 ,j1, j2, (j3,j4)
        self._home_system = {"home": None, "homx": None,
                             "homy": None, "homz": None, "homa": None, "homb": None}
        self._home_robot = {"x": None, "y": None,
    ...
    def _connect(self, port_name, file_init):
        try:
            ...
            job = [
                [{"command": "g2core", "prm": "{line:n}"}, 
				 {"command": "g2core", "prm": "{id: n, fv:n}"}],
                [{"command": "g2core", "prm": "{sr: n}"}],
                [{"command": "set_toolhead", "prm": {"x": self._config["toolhead"]["x"]}}, 
				 {"command": "move", "prm": {"path": "joint", "movement": 1, "speed": self._config["default_speed"]["joint"], "j0": 0, "jerk": list(self._config["default_jerk"]["joint"])}}, {"command": "g2core", "prm": "{tt32:n}"}, 
				 {"command": "set_motion", "prm": self._config["motion"]}]
            ]
            ...
            if file_init:
                try:
                    ...
                    # form g2core
                    job.append([{"command": "g2core", "prm": "c"}
                               for c in content])
                except:
                    pass
            ...
            for i, j in enumerate(job):
                ...
                self._connect_percentage(
                    _init_num[i], _init_num[-1], result, 5 + time.time() + 2*len(result))
    ...
    def _connect_backup(self, port_name):
        try:
            ...
            job = [
                ...
                [{"command": "g2core", "prm": "{di1fn:4}"}, 
                 {"command": "g2core", "prm": "{ej: 1}"}, 
                 {"command": "g2core", "prm": "{jv:4}"}, 
                 {"command": "g2core", "prm": "{sv:1}"}, 
                 {"command": "g2core", "prm": "{si:200}"}, 
                 {"command": "g2core", "prm": "{qv:2}"}, 
                 {"command": "g2core", "prm": "{xam:1}"}, 
                 {"command": "g2core", "prm": "{yam:1}"}, 
                 {"command": "g2core", "prm": "{zam:1}"}, 
                 {"command": "g2core", "prm": "{aam:1}"}, 
                 {"command": "g2core", "prm": "{bam:1}"}, 
                 {"command": "g2core", "prm": "{cam:1}"}, 
                 {"command": "g2core", "prm": "{1:{sa:1.8}}"}, 
                 {"command": "g2core", "prm": "{2:{sa:1.8}}"}, 
                 {"command": "g2core", "prm": "{3:{sa:1.8}}"}, 
                 {"command": "g2core", "prm": "{4:{sa:1.8}}"}, 
                 {"command": "g2core", "prm": "{5:{sa:1.8}}"}, 
                 {"command": "g2core", "prm": "{6:{sa:1.8}}"}, 
                 {"command": "g2core", "prm": "{1:{tr:45}}"}, 
                 {"command": "g2core", "prm": "{2:{tr:18}}"}, 
                 {"command": "g2core", "prm": "{3:{tr:18}}"}, 
                 {"command": "g2core", "prm": "{4:{tr:90}}"}, 
                 {"command": "g2core", "prm": "{5:{tr:90}}"}, 
                 {"command": "g2core", "prm": "{6:{tr:1.3535433}}"}, 
                 {"command": "g2core", "prm": "{1:{mi:32}}"}, 
                 {"command": "g2core", "prm": "{2:{mi:32}}"}, 
                 {"command": "g2core", "prm": "{3:{mi:32}}"}, 
                 {"command": "g2core", "prm": "{4:{mi:32}}"}, 
                 {"command": "g2core", "prm": "{5:{mi:32}}"}, 
                 {"command": "g2core", "prm": "{6:{mi:32}}"}, 
                 {"command": "g2core", "prm": "{1:{ma:0}}"}, 
                 {"command": "g2core", "prm": "{2:{ma:1}}"}, 
                 {"command": "g2core", "prm": "{3:{ma:2}}"}, 
                 {"command": "g2core", "prm": "{4:{ma:3}}"}, 
                 {"command": "g2core", "prm": "{5:{ma:4}}"}, 
                 {"command": "g2core", "prm": "{6:{ma:5}}"}, 
                 {"command": "g2core", "prm": "{1:{po:0}}"}, 
                 {"command": "g2core", "prm": "{2:{po:1}}"}, 
                 {"command": "g2core", "prm": "{3:{po:0}}"}, 
                 {"command": "g2core", "prm": "{4:{po:1}}"}, 
                 {"command": "g2core", "prm": "{5:{po:1}}"}, 
                 {"command": "g2core", "prm": "{6:{po:0}}"}, 
                 {"command": "g2core", "prm": "{1:{pm:1}}"}, 
                 {"command": "g2core", "prm": "{2:{pm:1}}"}, 
                 {"command": "g2core", "prm": "{3:{pm:1}}"}, 
                 {"command": "g2core", "prm": "{4:{pm:1}}"}, 
                 {"command": "g2core", "prm": "{5:{pm:1}}"}, 
                 {"command": "g2core", "prm": "{6:{pm:1}}"}, 
                 {"command": "g2core", "prm": "{1:{pl:1.0}}"}, 
                 {"command": "g2core", "prm": "{2:{pl:1.0}}"}, 
                 {"command": "g2core", "prm": "{3:{pl:1.0}}"}, 
                 {"command": "g2core", "prm": "{4:{pl:1.0}}"}, 
                 {"command": "g2core", "prm": "{5:{pl:1.0}}"}, 
                 {"command": "g2core", "prm": "{6:{pl:1.0}}"}, 
                 {"command": "g2core", "prm": "{xtn:1}"}, 
                 {"command": "g2core", "prm": "{xtm:1000}"}, 
                 {"command": "g2core", "prm": "{xhi:6}"}, 
                 {"command": "g2core", "prm": "{xhd:0}"}, 
                 {"command": "g2core", "prm": "{xsv:4000}"}, 
                 {"command": "g2core", "prm": "{xlv:500}"}, 
                 {"command": "g2core", "prm": "{xlb:50}"}, 
                 {"command": "g2core", "prm": "{xzb:69}"}, 
                 {"command": "g2core", "prm": "{ytn:0}"}, 
                 {"command": "g2core", "prm": "{ytm:420}"}, 
                 {"command": "g2core", "prm": "{yhi:2}"}, 
                 {"command": "g2core", "prm": "{yhd:0}"}, 
                 {"command": "g2core", "prm": "{ysv:4000}"}, 
                 {"command": "g2core", "prm": "{ylv:500}"}, 
                 {"command": "g2core", "prm": "{ylb:50}"}, 
                 {"command": "g2core", "prm": "{yzb:90}"}, 
                 {"command": "g2core", "prm": "{ztn:100}"}, 
                 {"command": "g2core", "prm": "{ztm:1500}"}, 
                 {"command": "g2core", "prm": "{zhi:3}"}, 
                 {"command": "g2core", "prm": "{zhd:1}"}, 
                 {"command": "g2core", "prm": "{zsv:4000}"}, 
                 {"command": "g2core", "prm": "{zlv:500}"}, 
                 {"command": "g2core", "prm": "{zlb:50}"}, 
                 {"command": "g2core", "prm": "{zzb:82}"}, 
                 {"command": "g2core", "prm": "{atn:-1000}"}, 
                 {"command": "g2core", "prm": "{btn:-1000}"}, 
                 {"command": "g2core", "prm": "{atm:0}"}, 
                 {"command": "g2core", "prm": "{btm:0}"}, 
                 {"command": "g2core", "prm": "{ahi:4}"}, 
                 {"command": "g2core", "prm": "{bhi:5}"}, 
                 {"command": "g2core", "prm": "{ahd:1}"}, 
                 {"command": "g2core", "prm": "{bhd:1}"}, 
                 {"command": "g2core", "prm": "{asv:10000}"}, 
                 {"command": "g2core", "prm": "{bsv:10000}"}, 
                 {"command": "g2core", "prm": "{alv:5000}"}, 
                 {"command": "g2core", "prm": "{blv:5000}"}, 
                 {"command": "g2core", "prm": "{alb:30}"}, 
                 {"command": "g2core", "prm": "{blb:30}"}, 
                 {"command": "g2core", "prm": "{azb:60}"}, 
                 {"command": "g2core", "prm": "{bzb:175}"}, 
                 {"command": "g2core", "prm": "{xvm:30000}"}, 
                 {"command": "g2core", "prm": "{yvm:30000}"}, 
                 {"command": "g2core", "prm": "{zvm:30000}"}, 
                 {"command": "g2core", "prm": "{avm:30000}"}, 
                 {"command": "g2core", "prm": "{bvm:30000}"}, 
                 {"command": "g2core", "prm": "{cvm:30000}"}, 
                 {"command": "g2core", "prm": "{xfr:30000}"}, 
                 {"command": "g2core", "prm": "{yfr:30000}"}, 
                 {"command": "g2core", "prm": "{zfr:30000}"}, 
                 {"command": "g2core", "prm": "{afr:30000}"}, 
                 {"command": "g2core", "prm": "{bfr:30000}"}, 
                 {"command": "g2core", "prm": "{cfr:30000}"}],
                 
                [{"command": "g2core", "prm": "{sr: n}"}],
                [{"command": "set_toolhead", "prm": {"x": self._config["toolhead"]["x"]}}, {"command": "move", "prm": {"path": "joint", "movement": 1, "speed": self._config["default_speed"]
                                                                                                                       ["joint"], "j0": 0, "jerk": list(self._config["default_jerk"]["joint"])}}, {"command": "g2core", "prm": "{tt32:n}"}, {"command": "set_motion", "prm": self._config["motion"]}]
            ]
    ...
    def _home_joint_3_4(self):
        ...
        # probe toward j4 -> 360
        _result = self.probe(
            {"j"+str(_input[-1]): 360, "speed": 5000}, append=True)
        if _result == None:
            return None
        ...
        command = {"command": "move", "prm": {
            "path": "joint", "movement": 0, "j4": 0, "speed": 5000}}
        result = self.play(command, False)
        result = json.loads(result)
        if len(result) == 0:
            return None
    ...
    def _home_joint(self, joint):
        ...
        # calibration
        if joint in ["j3", "j4"]:
            clb = {"j3": self._config["calibrate"]["j3"],
                   "j4": self._config["calibrate"]["j4"]}
        else:
    ...
    def probe(self, prm, append=False):
        ...
        if self._system["probe"]["e"]:
            try:
                _probe_travel = [self._system["probe"][k]
    ...
    def calibrate(self, prm):
        ...
        # update config
        if "j0" in prm:
            self._config["calibrate"]["j0"] = self._config["calibrate"]["j0"] + \
                prm["j0"] - joint[0]
            #_config_tmp["calibrate"]["j0"] = self._config["calibrate"]["j0"]
        if "j1" in prm:
            self._config["calibrate"]["j1"] = self._config["calibrate"]["j1"] + \
                prm["j1"] - joint[1]
            #_config_tmp["calibrate"]["j1"] = self._config["calibrate"]["j1"]
        if "j2" in prm:
            self._config["calibrate"]["j2"] = self._config["calibrate"]["j2"] + \
                prm["j2"] - joint[2]
            #_config_tmp["calibrate"]["j2"] = self._config["calibrate"]["j2"]
        if "j3" in prm:
            self._config["calibrate"]["j3"] = (
                self._config["calibrate"]["j3"] + prm["j3"] - joint[3]) % 360
            #_config_tmp["calibrate"]["j3"] = self._config["calibrate"]["j3"]
        if "j4" in prm:
            self._config["calibrate"]["j4"] = (
                self._config["calibrate"]["j4"] + prm["j4"] - joint[4]) % 360
    ...
    def _append_commands(self, commands):
        command_id_list = list(
            range(self._system["command_id"]+1, self._system["command_id"]+len(commands)+1))
        # append M2
        ...
        self._log_add([{"id": cmd["id"], "key": cmd["key"], "state": cmd["state"]}
                      for cmd in f_commands], "line_update")
    ...
    def _flush_commands(self, halt=True):
        if halt:
            ...
            # send jobs to log
            self._log_add([{"id": cmd["id"], "key":cmd["key"], "state":-1}
                          for X in _command_list for cmd in X], "line_update")
            """
			???
            ...
            if self._system["command"][2]:
                self._system["travel_final"] = np.copy(
                    self._system["command"][2][0]["travel_final"])
            elif self._system["command"][3]:
                self._system["travel_final"] = np.copy(
                    self._system["command"][3][0]["travel_final"])
            elif self._system["command"][4]:
                self._system["travel_final"] = np.copy(
                    self._system["command"][4][0]["travel_final"])
            elif self._system["command"][5]:
                self._system["travel_final"] = np.copy(
                    self._system["command"][5][0]["travel_final"])
            else:
                self._system["travel_final"] = np.copy(self._travel)
    ...
    def _command_compile(self):
        ...
        # {'gc_list', 'travel_final', "status", "message"}
        gcs = self._json_to_method(_command)
        # system is synced
        # if not synced then this command is already in the canceled
        if _command["id"] == self._system["command"][0][0]["id"]:
            ...
            # add to log ???
            self._log_add(
                [{"id": _command["id"], "key": _command["key"], "state": -1}], "line_update")
            return False
        ...
        if "status" in gcs:
            ...
            if not _command["fulfill"] and _command["error"]:
                ...
                for command in commands:
                    command["message"] = "canceled because of an error in the command with the id : " + \
                        str(_command["id"])
                #_cmd_log = [{"id": cmd["id"], "key": cmd["key"], "state": -1} for cmd in commands]
                self._log_add([{"id": cmd["id"], "key": cmd["key"], "state": -1}
                              for cmd in commands], "line_update")
                return False
                """
				???
				# add _command
        ...
        # add to log ???
        self._log_add([{"id": _command["id"], "key": _command["key"],
                      "state": _command["state"]}], "line_update")
            ...
            if self._device["connection"] > 0:
                ...
                try:
                    ...
                    try:
                        ...
                        self._log_add(
                            [{"id": _command["id"], "key": _command["key"], "state": _command["state"]}], "line_update")
                    except:
                        pass
    ...
    def _process_gc(self, gc):
        ...
        self._system["lines_to_send"] -= len(
            [x for x in line if x not in ['%', '~', '!']])
        if gc["gc"] == "%":
            self._system["lines_to_send"] = 4
            _printx(self._prnt, "lines to send: ",
                    self._system["lines_to_send"])
    ...
    def _process_response(self, response):
        # response
        if 'r' in response:
            self._system["lines_to_send"] = min(
                4, self._system["lines_to_send"] + 1)
            #_r = dict(response["r"])
    ...
    def _travel_update(self, response):
        mapping = {"posx": 0, "posy": 1, "posz": 2,
                   "posa": 3, "posb": 4, "posc": 5}
            ...
            if response["line"] % 2 == 0:
                ...
                try:
                    while self._system["command"][4][0]["id"] < command_id:
                        ...
                        # add to log
                        _command_update.append(
                            {"id": _command["id"], "state": 5, "key": _command["key"]})
                except:
                    pass
                ...
                try:
                    while self._system["command"][3][0]["id"] == command_id:
                        ...
                        # add to log
                        _command_update.append(
                            {"id": _command["id"], "state": 4, "key": _command["key"]})
                except:
                    pass
    ...
    def _home_update(self, response):
        ...
        update = False
        if "tt32" in response:
            home_key = list(set(response["tt32"].keys()) & set(
                self._home_robot.keys()))
    ...
    def _init_config(self):
        ...
        if self._config["unit"]["length"] == "mm":
            # speed_xyz
            self._config["default_speed"]["xyz"] = self._mm_to_inch(
                self._config["default_speed"]["xyz"])
            # jerk_xyz
            self._config["default_jerk"]["xyz"] = self._jerk_mm_to_inch(
                self._config["default_jerk"]["xyz"])
            # toolhead
            for k in self._config["toolhead"]:
                self._config["toolhead"][k] = self._mm_to_inch(
                    self._config["toolhead"][k])
    ...
    def config(self, prm=None):
        ...
        if tmp_config["unit"]["length"] == "mm":
            # speed_xyz
            tmp_config["default_speed"]["xyz"] = self._inch_to_mm(
                tmp_config["default_speed"]["xyz"])

            # jerk_xyz
            tmp_config["default_jerk"]["xyz"] = self._jerk_inch_to_mm(
                tmp_config["default_jerk"]["xyz"])
            # toolhead
            for k in tmp_config["toolhead"]:
                tmp_config["toolhead"][k] = self._inch_to_mm(
                    tmp_config["toolhead"][k])
    ...
    def set_default_jerk(self, prm):
        ...
        if "joint" in prm:
            if type(prm["joint"]) == list and len(prm["joint"]) == self._config["axis"]["number"]:
                self._config["default_jerk"]["joint"][0:self._config["axis"]
                                                      ["number"]] = prm["joint"]

        if "xyz" in prm:
            if type(prm["xyz"]) == list and len(prm["xyz"]) == self._config["axis"]["number"]:
                if self._config["unit"]["length"] == "mm":
                    self._config["default_jerk"]["xyz"][0:self._config["axis"]
                                                        ["number"]] = self._jerk_mm_to_inch(prm["xyz"])
                else:
                    self._config["default_jerk"]["xyz"][0:self._config["axis"]
                                                        ["number"]] = prm["xyz"]
    ...
    def _limit_check(self, joint):
        limit_passed = [i for i in range(len(self._config["limit"])) if not(
            self._config["limit"]["j" + str(i)][0] <= joint[i] <= self._config["limit"]["j" + str(i)][1])]
        status = 0
        if limit_passed:
            status = 100
    ...
    def f_k(self, joint):
        try:
            ...
            tmp = self._bx + self._l1 * math.cos(teta[1]) + self._l2 * math.cos(
                teta[1] + teta[2]) + self._config["toolhead"]["x"] * math.cos(teta[1] + teta[2] + teta[3])
            x = tmp * math.cos(teta[0])
            y = tmp * math.sin(teta[0])
            z = self._bz + self._l1 * math.sin(teta[1]) + self._l2 * math.sin(
                teta[1] + teta[2]) + self._config["toolhead"]["x"] * math.sin(teta[1] + teta[2] + teta[3])
    ...
    def i_k(self, xyz):
        try:
            ...
            if L > (self._l1 + self._l2) or self._l1 > (self._l2 + L) or self._l2 > (self._l1 + L):
                return None

            # init status
            status = 0
            # in this case there is no solution
            if L > (self._l1 + self._l2) - self._delta_e or self._l1 > (self._l2 + L) - self._delta_e:
                status = 1

            teta_l1_L = math.acos(
                (self._l1 ** 2 + L ** 2 - self._l2 ** 2) / (2 * self._l1 * L))  # l1 angle to L
            ...
            teta_l1_l2 = math.acos(
                (self._l1 ** 2 + self._l2 ** 2 - L ** 2) / (2 * self._l1 * self._l2))  # l1 angle to l2
    ...
    def _joint_to_xyz(self, joint):
        ...
        # first we find x, y, z assuming base rotation is zero (teta_0 = 0). Then we rotate everything
        # then we rotate the robot around z axis for teta_0
        tmp = self._bx + self._l1 * math.cos(teta_1) + self._l2 * math.cos(
            teta_1 + teta_2) + self._config["toolhead"]["x"] * math.cos(teta_1 + teta_2 + teta_3)
        x = tmp * math.cos(teta_0)
        y = tmp * math.sin(teta_0)
        z = self._bz + self._l1 * math.sin(teta_1) + self._l2 * math.sin(
            teta_1 + teta_2) + self._config["toolhead"]["x"] * math.sin(teta_1 + teta_2 + teta_3)
        ...
        if len(joint) == 6:
            # [x, y, z, alpha, beta, joints[5]]
            return np.array([x, y, z, alpha, beta, joint[5]])
        else:
    ...
    def _xyz_to_joint(self, xyz):
        ...
        teta_1 = teta_l1_L + teta_L_x
        # note that the other solution would be to set teta_1 = teta_L_x - teta_l1_L. But for the dynamics of the robot the first solution works better.
        teta_l1_l2 = math.acos((self._l1 ** 2 + self._l2 ** 2 - L ** 2) /
                               (2 * self._l1 * self._l2))  # l1 angle to l2
        teta_2 = teta_l1_l2 - math.pi
        teta_3 = alpha - teta_1 - teta_2
        teta_4 = beta
        teta_0 = math.degrees(teta_0)
    ...
    def _joint_validate(self, joint_init, joint_final):
        ...
        for i in range(len(self._config["limit"])):
            ...
            if joint_init[i] > self._config["limit"]["j" + str(i)][1]:
                # above
                joint_achieve[i] = max(
                    joint_final[i], self._config["limit"]["j" + str(i)][0])
                message = "initial position (joint: " + \
                    str(joint_init)+") is out of limit"
                status = 100

            elif self._config["limit"]["j" + str(i)][0] <= joint_init[i] <= self._config["limit"]["j" + str(i)][1]:
                # between
                joint_achieve[i] = min(max(joint_final[i], self._config["limit"]
                                       ["j" + str(i)][0]), self._config["limit"]["j" + str(i)][1])
            else:
                # under
                joint_achieve[i] = min(
                    joint_final[i], self._config["limit"]["j" + str(i)][1])
                message = "initial position (joint: " + \
                    str(joint_init)+") is out of limit"
                status = 0

        if not np.array_equal(joint_achieve, joint_final):
            message = "final position (joint: " + \
                str(joint_final)+") is out of limit"
            status = 100
    ...
    def _joint_validate_backup(self, joint_init, joint_final):
        ...
        # find the achieve
        for i in range(len(self._config["limit"])):
            joint_achieve[i] = min(max(self._config["limit"]["j" + str(i)]
                                   [0], joint_achieve[i]), self._config["limit"]["j" + str(i)][1])
    ...
    def _line_validate(self, xyz_init, xyz_final):
        ...
        for i in range(1, L+1):
            ...
            if status or self._limit_check(joint)["status"]:
                xyz_r = xyz
                for i in range(10):
                    [xyz_achieve, xyz_r] = self._xyz_achieve(
                        xyz_achieve, xyz_r)

                message = "achievable position is (xyz: " + \
                    str(xyz_achieve) + ")"
    ...
    def _move_to_gc(self, travel_final, prm):
        gc = 'X{:07.4f} Y{:07.4f} Z{:07.4f} A{:07.4f} B{:07.4f} C{:07.4f}'.format(
            travel_final[0], travel_final[1], travel_final[2], travel_final[3], travel_final[4], travel_final[5])
    ...
    def _move_joint(self, prm):
        ...
        joint_init = self._travel_to_joint(
            np.copy(self._system["travel_final"]))
    ...
    def _set_joint(self, prm):
        joint_current = self._travel_to_joint(
            np.copy(self._system["travel_final"]))
    ...
    def _set_io(self, prm):
        valid_m100 = ["di1mo", "di2mo", "di3mo", "di4mo", "do1mo", "do2mo",
                      "do3mo", "do4mo", "out1", "out2", "out3", "out4", "out5"]
        cmd = []
